...
def filter_vocab_embed(ds=None, dim=300):
    ...
    all_set = set()
    # 将训练集和测试集中sentence的所有word存入set
    for f in f_list:
        for line in f:
            line = line.lower().strip('\n').split('\t')[1]  # 只提取sentence列
            words = line.split(' ')
            for w in words:
                all_set.add(w)
    ...
    scale = np.sqrt(3.0/dim)  # 0.1
    embedding_matrix[-1] = np.random.uniform(-1*scale, scale, dim)  # 最后一列从-0.1~0.1随机生成300个
    ...
    pickle.dump(vocab_dict, open(os.path.join('data', ds, 'vocabulary_bak.pkl'), 'wb'))
    np.save(os.path.join('data', ds, 'embedding_table_bak.npy'), embedding_matrix)
    print("miss:%d" % (vocab_size - hit))

    # vocabulary.pkl  -  word：word_idx
    # embedding_table.npy  - embedding矩阵（3977 x 300），embedding_matrix[word_idx] = word对应的glove embedding
